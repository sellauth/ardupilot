...
const AP_Param::GroupInfo AP_MotorsMulticopter::var_info[] = {
    ...
    // @Param: ENGINE_FAIL_NUM
    // @DisplayName: Motor Failure Number
    // @Description: Number of motors to simulate as failed
    // @Range: 0 8
    // @Units: Number
    // @User: Advanced
    AP_GROUPINFO("FAIL_NUM", 45, AP_MotorsMulticopter, _engine_fail_num, 0),

    // @Param: ENGINE_FAIL_PCT
    // @DisplayName: Motor Failure Percentage
    // @Description: Percentage of motor failure to simulate
    // @Range: 0 100
    // @Units: Percentage
    // @User: Advanced
    AP_GROUPINFO("FAIL_PCT", 46, AP_MotorsMulticopter, _engine_fail_pct, 0),

    AP_GROUPEND};

// Constructor
AP_MotorsMulticopter::AP_MotorsMulticopter(uint16_t speed_hz) : AP_Motors(speed_hz),
...
float AP_MotorsMulticopter::get_current_limit_max_throttle()
{
    ...
    if (_batt_current_max <= 0 || // return maximum if current limiting is disabled
        ...
        !battery.current_amps(_batt_current, batt_idx))
