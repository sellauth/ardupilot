...
void Copter::thrust_loss_check()
{
    ...
    const bool rpm_adequate = AP::esc_telem().are_motors_running(motor_mask, g2.takeoff_rpm_min, 0);

    // If the motors are not running, we may have a thrust loss
    if (telem_active && !rpm_adequate)
    {
        ...
        LOGGER_WRITE_ERROR(LogErrorSubsystem::THRUST_LOSS_CHECK, LogErrorCode::FAILSAFE_OCCURRED);
        // send message to gcs
        // NOTE: motors->get_lost_motor() is the motor with the highest throttle output
        gcs().send_text(MAV_SEVERITY_EMERGENCY, "ESC Potential Thrust Loss (%d)", (int)motors->get_lost_motor() + 1);
        // enable thrust loss handling
        motors->set_thrust_boost(true);
        // the motors library disables this when it is no longer needed to achieve the commanded output
    }
